...
class DDQN:
    ...
    def __init__(self, args, save_dir):
        ...
        if hasattr(args, 'double_deep'):
            self.double_deep = args.double_deep
        if hasattr(args, 'with_hrs'):
        ...
        self.augment = bool(int(config['car_agent']['augment']))
        ...
        if hasattr(args, 'epsilon_final'):
            self.epsilon_final = args.epsilon_final
        if hasattr(args, 'replay_buffer_size'):
            self.replay_buffer_size = args.replay_buffer_size
        else:
            self.replay_buffer_size = 5000
        if hasattr(args, 'replay_start_size'):
            self.replay_start_size = args.replay_start_size
        if hasattr(args, 'batch_size'):
            self.batch_size = args.batch_size
        self.tau = 1.0
        #
        if hasattr(args, 'nb_steps'):
            self.nb_steps = args.nb_steps
        if hasattr(args, 'nb_episodes'):
            self.nb_episodes = args.nb_episodes
        if hasattr(args, 'train_in_loop'):
            self.train_in_loop = args.train_in_loop
        if hasattr(args, 'nb_steps_per_train_iter'):
            self.nb_steps_per_train_iter = args.nb_steps_per_train_iter
        if hasattr(args, 'target_network_update'):
            self.target_network_update = args.target_network_update
        #
        # Create q-network
        self.agent = Agent(self.model_type, self.state_dim, self.act_dim, self.lr, self.tau, self.output_activation)
        # Memory Buffer for Experience Replay
        self.buffer = MemoryBuffer(self.replay_buffer_size)
        #
        self.processing = ImageProcessing(self.state_dim[1], self.state_dim[2], self.state_dim[0])
        #
    ...
    def train(self, env, summary_writer):
        ...
        results = []
        episodes, total_steps, train_count = 0, 0, 0
        ...
        # while total_steps < self.nb_steps:
        while episodes < self.nb_episodes:
            ...
            if(
                    math.isnan(cumul_reward) |
                    math.isnan(cumul_reward / episode_steps) |
                    math.isnan(episode_qs) |
                    math.isnan(episode_qs / episode_steps) |
                    math.isnan(self.epsilon)
            ):
                print('Error!!! Nan Value!!!')
                print('\n')
                print('Total Reward: {}'.format(cumul_reward))
                print('Average Reward: {}'.format(cumul_reward / episode_steps))
                print('Total Episode Q: {}'.format(episode_qs))
                ...
                summary_writer.add_summary(score, global_step=episodes)
                summary_writer.add_summary(avg_score, global_step=episodes)
                summary_writer.add_summary(qs, global_step=episodes)
                summary_writer.add_summary(avg_qs, global_step=episodes)
                summary_writer.add_summary(eps, global_step=episodes)
