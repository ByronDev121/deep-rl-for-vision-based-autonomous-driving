...
if not exists(log_dir):
    os.makedirs(log_dir)
checkpoints_dir = Path("logs/ppo2_AirsimCar_checkpoints/")
tensorboard_dir = Path("logs/ppo2_AirsimCar_tensorboard/")
best_model_dir = Path("logs/ppo2_AirsimCar_best_model")
eval_dir = Path("logs/ppo2_AirsimCar_eval_results")
...
eval_callback = EvalCallback(env, best_model_save_path=best_model_dir,
                             log_path=eval_dir, eval_freq=25000)

# Create the callback list
callbacks = CallbackList([checkpoint_callback, eval_callback])

model = PPO2(CnnPolicy, env, verbose=1, tensorboard_log=tensorboard_dir)
model.learn(total_timesteps=2000000, callback=callbacks, log_interval=10)
